...
namespace Exchange {
    ...
    private:
        ...
        MEOrdersAtPrice::OrdersAtPriceHashMap price_orders_at_price_;
    ...
    private:
        ...
        auto addOrder(MEOrder *order) noexcept {
            ...
            if (!orders_at_price) {
                ...
                auto new_orders_at_price = orders_at_price_pool_.allocate(order->side_, order->price_,
        ...
        auto removeOrder(MEOrder *order) noexcept {
            ...
            if (order->prev_order_ == order) { // only one element left so remove entire price level
