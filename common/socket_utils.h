...
namespace Common {
    ...
    inline auto setSOTimestamp(int fd) -> bool {
        int one = 1;
#ifdef __APPLE__
        return (setsockopt(fd, SOL_SOCKET, SO_TIMESTAMP_MONOTONIC, &one, sizeof(one)) != -1);
#else
    ...
    [[nodiscard]] inline auto createSocket(Logger &logger, const SocketCfg& socket_cfg) -> int {
        ...
        for (addrinfo *rp = result; rp; rp = rp->ai_next) {
            ...
            if (socket_cfg.is_listening_) {
                ...
                sockaddr_in addr{};  // Default-initialize to zero
                addr.sin_family = AF_INET;
                addr.sin_port = htons(socket_cfg.port_);
                addr.sin_addr.s_addr = htonl(INADDR_ANY);
                memset(&(addr.sin_zero), 0, sizeof(addr.sin_zero));  // Properly initialize sin_zero
